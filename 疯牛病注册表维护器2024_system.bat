...
rem 注册表自带项补丁
echo 修补注册表
for /f %%r in ('dir /b %~dp0data\fix-sys\*.reg') do (
echo 发现注册表补丁 %~dp0data\fix-sys\%%r
reg import %~dp0data\fix-sys\%%r
)
...
"%~dp0data\tools\wimlib-imagex.exe" extract "%wimdir%" 4 \Windows\System32\config\system --dest-dir="%~dp0data\temp" --nullglob --no-acls
reg load hklm\install-sys "%~dp0data\temp\system"
exit /b
